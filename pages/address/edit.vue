<template>
    <view class="page-container page-bottom">
        <u-navbar :is-back="false" title="编辑地址" title-size="32" title-color="#333" z-index="99999">
            <view class="navbar">
        ...
        <view class="address-form">
            ...
            <button class="page-button" @tap="$u.throttle(editAddress, 500)">确认</button>
        </view>
    </view>
</template>
        ...
        onLoad(data) {
            ...
            this.$common.getCustomer(this, data => {
				if (this.$common.isNotEmpty(data)) {
                    this.requestAddress(this.addressId);
				}
			}, true);
        },
        methods: {
            ...
            requestAddress(addressId) {
                this.$u.post('/getAddressById', {
                    addressId
                }).then(res => {
                    if (this.$common.isNotEmpty(res.data)) {
                        this.address = res.data;
                        this.address.customerImg = this.$common.pictureToArray(this.address.customerImg);
